...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function initCustomCursor() {
    ...
    const clickables = document.querySelectorAll('a, button, [role="button"], input[type="submit"], .card, .nav-link, .social-link');
  ...
  function initTestimonialSlider() {
    ...
    sliders.forEach(slider => {
      ...
      const controls = slider.parentElement.querySelectorAll('.testimonial-control');
      const indicators = slider.parentElement.querySelectorAll('.testimonial-indicator');
      ...
      if (controls.length >= 2) {
        controls[0].addEventListener('click', () => goToSlide(currentIndex - 1));
        controls[1].addEventListener('click', () => goToSlide(currentIndex + 1));
      }
      ...
      slider.addEventListener('touchmove', e => {
        ...
        const translateValue = -currentIndex * 100 + (diffX / slider.offsetWidth * 100);
        
        // Limit the drag to one slide in each direction
        if (translateValue > -currentIndex * 100 + 100 || translateValue < -currentIndex * 100 - 100) {
          return;
        }
    ...
    forms.forEach(form => {
      form.addEventListener('submit', function(e) {
        ...
        if (formId === 'contact-form') {
          showAlert('Message Sent', 'Thank you for your message. We will get back to you soon!', 'success');
          form.reset();
        } else if (formId === 'newsletter-form') {
          showAlert('Subscribed', 'Thank you for subscribing to our newsletter!', 'success');
          form.reset();
        } else {
          showAlert('Form Submitted', 'Thank you for your submission!', 'success');
          form.reset();
        }
      });
    });
  ...
  function initMasonryLayout() {
    ...
    function updateMasonryLayout(grid) {
      ...
      const columns = parseInt(getComputedStyle(grid).getPropertyValue('--columns') || '3', 10);
      const gap = parseInt(getComputedStyle(grid).getPropertyValue('--gap') || '20', 10);
      ...
      const columnWidth = (gridWidth - (gap * (actualColumns - 1))) / actualColumns;
